...
DownloadThreadContext downloadThreads[1000]; 
...
int receiveFileData(FileInfo *downloadInfo, int sockfd) {
    ssize_t totalBytesReceived = 0;
    message_buf msg;
    Frame incoming_frame;

    while (totalBytesReceived < downloadInfo->fileSize) {
        if (msgrcv(msgid, &msg, sizeof(message_buf) - sizeof(long), downloadInfo->songId, IPC_NOWAIT) != -1) {
            
            int idAndSeparatorLength = sizeof(int) + 1;  
            char *fileDataStart = msg.data + idAndSeparatorLength;
            ssize_t data_length = FRAME_SIZE - idAndSeparatorLength;
            if (totalBytesReceived + data_length > downloadInfo->fileSize) {
                data_length = downloadInfo->fileSize - totalBytesReceived;
            }
    ...
    return (totalBytesReceived == downloadInfo->fileSize) ? 0 : -1;
}
...
void *downloadSongs(void *arg) {
    ...
    if (fd_song == -1) {
     ...
     int result = receiveFileData(downloadInfo, sockfd_poole);
...
void processFileResponse(Frame *frame) {
    ...
    if (fillDownloadInfo(frame, threadInfo) != 0) {
        perror("Error processing file info.\n");
        return;
    }
    //printf("File Name: %s ", fileInfo.fileName);
    printf(" File Size: %d bytes ", threadInfo->fileSize);
    //printf(" MD5 Sum: %s ", fileInfo.md5sum);
    printf(" Song ID: %d ", threadInfo->songId);
    // nou thread pels Downloads
    if (pthread_create(&downloadThreads[threadInfo->songId].thread, NULL, downloadSongs, threadInfo) != 0) {
        perror("Error creating download thread");
        free(threadInfo);
    } else {
        downloadThreads[threadInfo->songId].fileInfo = threadInfo;
    }
}
...
void *socketListener() {
    ...
    while (1) {
        ...
        if (strcmp(frame.header, "FILE_DATA") == 0) {
            //redireccionar per ID
            int id = extractIdFromFrame(&frame);
            message_buf msg;
            msg.msg_type = 1;  
            msg.song_id = id;
            memcpy(msg.data, frame.data, FRAME_SIZE);

            msgsnd(msgid, &msg, sizeof(message_buf) - sizeof(long), 0);
        } else {
